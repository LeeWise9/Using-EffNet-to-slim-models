...
def eff_block(x_in, ch_in, ch_out):
    x = Conv2D(ch_in,kernel_size=(1, 1),padding='same',kernel_initializer='he_normal',use_bias=False)(x_in)
    x = LeakyReLU(alpha=0.3)(x)
    #x = BatchNormalization()(x)
    x = keras.layers.DepthwiseConv2D(kernel_size=(1, 3),padding='same',kernel_initializer='he_normal',use_bias=False)(x)
    x = LeakyReLU(alpha=0.3)(x)
    #x = BatchNormalization()(x)
    x = MaxPooling2D(pool_size=(2, 1),strides=(2, 1))(x)
    x = keras.layers.DepthwiseConv2D(kernel_size=(3, 1),padding='same',kernel_initializer='he_normal',use_bias=False)(x)
    x = LeakyReLU(alpha=0.3)(x)
    #x = BatchNormalization()(x)
    x = Conv2D(ch_out,kernel_size=(2, 1),strides=(1, 2),padding='same',kernel_initializer='he_normal',use_bias=False)(x)
    x = LeakyReLU(alpha=0.3)(x)
    #x = BatchNormalization()(x)
    x = Dropout(0.25)(x)
    return x
...
def batch_generator(x, y, batch_size, shape, training=True, data_dir='data2/', 
...
if __name__ == '__main__':
    ...
    model = eff_model(shape)
    print(model.summary())
    
    filename = 'model_eff'

    # 根据validation loss保存最优模型
    save_best = callbacks.ModelCheckpoint(filename+'.h5', monitor='val_loss', verbose=1, 
    ...
    history = model.fit_generator(batch_generator(X_train, y_train, batch_size, shape, 
                                                  data_dir=data_path, training=True, monitor=False),
    ...
    plt.savefig('train_val_loss_'+filename[6:]+'.jpg',dpi=600)
